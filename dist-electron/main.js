...
ipcMain.handle("dns-query", async (event, { domain, type }) => {
	...
	try {
		...
		if (error.code === "ENOTFOUND") errorMessage = "域名不存在或无法解析";
		else if (error.code === "ECONNREFUSED") errorMessage = "DNS 服务器连接被拒绝";
		else if (error.code === "ETIMEDOUT" || error.code === "ETIMEOUT") errorMessage = "DNS 查询超时";
		else if (error.code === "EAI_AGAIN") errorMessage = "DNS 服务暂时不可用，请稍后重试";
		return {
			success: false,
			domain,
			type,
...
ipcMain.handle("ping", async (event, { host, count, timeout }) => {
	...
	const pingTimeout = Math.min(Math.max(timeout || 8e3, 1e3), 3e4);
	try {
		let command;
		if (isWindows) command = `ping -n ${pingCount} -w ${pingTimeout} ${host}`;
		else command = `ping -c ${pingCount} -W ${Math.ceil(pingTimeout / 1e3)} ${host}`;
		const { stdout, stderr } = await execAsync(command, {
			timeout: pingTimeout * pingCount + 1e4,
			maxBuffer: 1024 * 1024,
			windowsHide: true,
			encoding: "buffer"
		});
		let output;
		if (isWindows) output = import_lib.default.decode(stdout || stderr, "gbk");
		else output = (stdout || stderr).toString();
		return {
			success: true,
			output,
			isWindows,
		...
		let output = "";
		if (isWindows) output = import_lib.default.decode(error.stdout || error.stderr || Buffer.from(""), "gbk");
		else output = (error.stdout || error.stderr || "").toString();
		if (output.includes("请求找不到主机") || output.includes("could not find host") || output.includes("Name or service not known") || output.includes("non-existent domain")) return {
			success: false,
		...
		if (output.includes("无法访问目标主机") || output.includes("Destination Host Unreachable") || output.includes("Network is unreachable")) return {
			success: false,
